...
func renderNodes(b *strings.Builder, nodes []Node, r *EvalResult) error {
	for _, n := range nodes {
		...
		case InputNode:
			...
			renderInput(b, x, v)
		case CalcNode:
			// Calculations are intentionally invisible unless interpolated or shown.
		case ShowNode:
...
async function md0Refresh(){const values={};document.querySelectorAll('[data-md0-input]').forEach(el=>{values[el.name]=el.type==='checkbox'?String(el.checked):el.value});const r=await fetch('/render',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(values)});const text=await r.text();if(!r.ok){console.error(text);return}document.getElementById('md0-document').innerHTML=text}
`
