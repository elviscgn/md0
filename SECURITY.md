...
Every page load receives a fresh cryptographically random 256-bit capability token and a separate `ReactiveSession`. `POST /render` must present that token in `X-MD0-Token`, so independent browser tabs do not share reactive state. The session store is bounded to 32 live sessions and evicts the oldest session at capacity.
